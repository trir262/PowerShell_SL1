    ...
    Describe 'Get-SL1Discovery' {
        BeforeAll {
            ...
            Mock Invoke-SL1Request { 
                return New-Object -TypeName psobject -Property @{ 
                    StatusCode = [System.Net.HttpStatusCode]::OK
                    Content = $global:JSONs['discovery_sessionID']
                } 
            } -ParameterFilter { $Uri -and $Uri -match '/api/discovery_session/\d+$' }
            
            #Mock for /api/appliance?limit
            Mock Invoke-SL1Request { 
                return New-Object -TypeName psobject -Property @{ 
                    StatusCode = [System.Net.HttpStatusCode]::OK
                    Content = ($global:JSONs['discovery_sessionquery'])
                }
            } -ParameterFilter { $Uri -and $Uri -eq "$($url)/api/discovery_session?$($Filter)&limit=$($Limit)"}

            #Mock for /api/organization? contains=customerX&extended_fetch=1
            Mock Invoke-SL1Request { 
                return New-Object -TypeName psobject -Property @{ 
                    StatusCode = [System.Net.HttpStatusCode]::OK
                    Content = $global:JSONs['discovery_sessionxtend']
                } # /api/organization?$($Filter)&limit=$($Limit)&hide_filterinfo=1&extended_fetch=1
            } -ParameterFilter { $Uri -and $Uri -eq "$($Url)/api/discovery_session?$($Filter)&limit=$($limit)&hide_filterinfo=1&extended_fetch=1"}

            Connect-SL1 -Uri $Url -Credential $GoodCred
            $Cmd = 'Get-SL1Discovery'
        }
        ...
        Context 'Testing Id Parameter' {
            It 'Parameter Id is higher than 0 returns Discovery Session' {
                Get-SL1Discovery -Id 1 | should -not -be $null
                Assert-MockCalled -CommandName Invoke-SL1Request -Times 1 -Scope It
            }

            It 'Parameter Id is below 0 throws error' {
                { Get-SL1Discovery -Id -5} | Should -Throw
                Assert-MockCalled -CommandName Invoke-SL1Request -Times 0 -Scope It
            }
        }

        Context 'Testing Filter Parameter' {
            It 'Parameter Filter returns 1 Discovery Session' {
                (Get-SL1Discovery -Filter $Filter).name | Should -be 'discovery'
