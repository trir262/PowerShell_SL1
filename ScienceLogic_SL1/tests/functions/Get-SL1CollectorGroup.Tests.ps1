...
InModuleScope 'ScienceLogic_SL1' {
    Describe 'Get-SL1CollectorGroup' {
        BeforeAll {
            ...
            Mock Invoke-SL1Request { 
                return New-Object -TypeName psobject -Property @{ 
                    StatusCode = [System.Net.HttpStatusCode]::OK
                    Content = $global:JSONs['collector_groupID']
                } 
            } -ParameterFilter { $Uri -and $Uri -match '/api/collector_group/\d+$' }
            
            #Mock for /api/appliance?limit
            Mock Invoke-SL1Request { 
                return New-Object -TypeName psobject -Property @{ 
                    StatusCode = [System.Net.HttpStatusCode]::OK
                    Content = ($global:JSONs['collector_groupquery'])
                }
            } -ParameterFilter { $Uri -and $Uri -eq "$($url)/api/collector_group?$($ApplianceFilter)&limit=$($Limit)"}

            #Mock for /api/organization? contains=customerX&extended_fetch=1
            Mock Invoke-SL1Request { 
                return New-Object -TypeName psobject -Property @{ 
                    StatusCode = [System.Net.HttpStatusCode]::OK
                    Content = $global:JSONs['collector_groupxtend']
                } # /api/organization?$($Filter)&limit=$($Limit)&hide_filterinfo=1&extended_fetch=1
            } -ParameterFilter { $Uri -and $Uri -eq "$($Url)/api/collector_group?$($ApplianceFilter)&limit=$($limit)&hide_filterinfo=1&extended_fetch=1"}

            Connect-SL1 -Uri $Url -Credential $GoodCred
            $Cmd = 'Get-SL1CollectorGroup'
        }
        ...
        Context 'Testing Id Parameter' {
            It 'Parameter Id is higher than 0 returns collector_group' {
                Get-SL1CollectorGroup -Id 1 | should -not -be $null
                Assert-MockCalled -CommandName Invoke-SL1Request -Times 1 -Scope It
            }

            It 'Parameter Id is below 0 throws error' {
                { Get-SL1CollectorGroup -Id -5} | Should -Throw
                Assert-MockCalled -CommandName Invoke-SL1Request -Times 0 -Scope It
            }
        }

        Context 'Testing Filter Parameter' {
            It 'Parameter Filter returns 1 collector group' {
                (Get-SL1CollectorGroup -Filter $ApplianceFilter).cug_name | Should -be 'colgroup'
